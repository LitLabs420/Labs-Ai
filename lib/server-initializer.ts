...
class ServerInitializer {
  ...
  private deepseekClient: any | null = null;
  ...
  async initialize(): Promise<InitializationStatus> {
    ...
    try {
      ...
      try {
        warnings.push('OpenAI package not installed - feature disabled');
      ...
      // Initialize DeepSeek (DISABLED - requires OpenAI SDK)
      try {
        warnings.push('DeepSeek disabled - OpenAI SDK not installed');
