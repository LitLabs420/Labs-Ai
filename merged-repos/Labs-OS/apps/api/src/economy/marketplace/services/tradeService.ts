...
import { publish } from "../../../lib/natsBus";
...
export async function requestTrade(params: {
  ...
  // Redis is a fast-path; DB idempotency is enforced at route layer
  const idemKey = `trade:idem:${params.idempotencyKey}`;
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    await redis.set(`escrow:${trade.id}`, "1", "EX", ESCROW_TTL_SECONDS);
...
export async function settleTrade(tradeId: string) {
  const escrowKey = `escrow:${tradeId}`;
