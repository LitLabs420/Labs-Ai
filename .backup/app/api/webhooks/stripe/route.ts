...
import { auditLog } from '@/lib/security-utils';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    switch (event.type) {
      ...
      // Silently ignore unhandled event types (don't log to avoid spam)
      default:
        break;
    }
...
async function logFailedPayment(data: {
  ...
  try {
    ...
    if (supabaseError) {
      captureError('Failed to log payment failure to Supabase', { error: supabaseError });
      return;
    }
  } catch (error: unknown) {
    captureError('Error logging failed payment', {
      error: error instanceof Error ? error.message : String(error),
...
async function cancelSubscription(data: {
  ...
  try {
    ...
    try {
      ...
      // Check if customer is a real customer object (not deleted) and has an email
      if (customer && 'email' in customer && customer.email && !customer.deleted) {
        const emailResponse = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/email/send`, {
          ...
          body: JSON.stringify({
            ...
            data: {
              ...
              tier_name: subscription.metadata?.tier_name || 'Creator',
              cancellation_date: new Date().toLocaleDateString(),
            },
          }),
        });
        if (emailResponse.ok) console.log('✓ Cancellation email sent');
      }
    } catch (emailError) {
        ...
        const emailResponse = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/email/send`, {
          ...
          body: JSON.stringify({
            ...
            data: {
              ...
              amount: (data.amountDue / 100).toFixed(2),
              payment_date: new Date().toLocaleDateString(),
            },
          }),
        });
        if (emailResponse.ok) console.log('✓ Invoice failure notification sent');
      }
    } catch (emailError) {
      console.error('Failed to send invoice payment failure email:', emailError);
    }
  } catch (error: unknown) {
